...
# Buscamos donde empieza el try del boton "Procesar y Cargar"
idx_start = content.find('if arch.name.endswith(\'.xlsx\'):')
if idx_start != -1:
    idx_end = content.find('bar.progress(1.0)', idx_start)
    if idx_end != -1:
        # Reemplazar todo este bloque
        old_block = content[idx_start : idx_end + len('bar.progress(1.0)')]
        
        new_block = """if arch.name.endswith('.xlsx'):
        ...
        with open('app.py', 'w', encoding='utf-8') as f:
            f.write(content)
        print("Bloque parser reescrito.")
    else:
        print("Fin no encontrado")
else:
